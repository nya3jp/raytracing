...
fn c(r: f64, g: f64, b: f64) -> Box<SolidColor> {
    Box::new(SolidColor::new(Color::new(r, g, b)))
}
            ...
            vec![
                ...
                PlainObject::new_box(
                    ...
                    Lambertian::new(Box::new(Checker::new(
                        c(0.5, 0.5, 0.5),
                        SolidColor::new_box(Color::WHITE),
                        0.5,
                    ))),
                ),
            ],
    ...
    pub fn image14(aspect_ratio: f64, _rng: &mut Rng) -> (Camera, Objects) {
        ...
        let objects = Objects::new(
            vec![
                ...
                PlainObject::new_box(
                    Sphere::new(v(0.0, 0.0, -1.0), 0.5),
                    Dielectric::new(SolidColor::new_box(Color::WHITE), 1.5),
                ),
                PlainObject::new_box(
                    Sphere::new(v(-1.0, 0.0, -1.0), 0.5),
                    Dielectric::new(SolidColor::new_box(Color::WHITE), 1.5),
        ...
        for a in -11..11 {
            for b in -11..11 {
                ...
                balls.push(if choose_mat < 0.8 {
                    ...
                    PlainObject::new_box(shape, Lambertian::new(SolidColor::new_box(albedo)))
                } else if choose_mat < 0.95 {
                    let albedo = Color::random(rng) * 0.5 + Color::new(0.5, 0.5, 0.5);
                    let fuzz = rng.gen_range(0.0..0.5);
                    PlainObject::new_box(shape, Metal::new(SolidColor::new_box(albedo), fuzz))
                } else {
                    PlainObject::new_box(
                        shape,
                        Dielectric::new(SolidColor::new_box(Color::WHITE), 1.5),
            ...
            for b in -11..11 {
                ...
                balls.push(if choose_mat < 0.8 {
                    ...
                    PlainObject::new_box(
                        ...
                        Lambertian::new(SolidColor::new_box(albedo)),
                    )
                } else if choose_mat < 0.95 {
                    let albedo = Color::random(rng) * 0.5 + Color::new(0.5, 0.5, 0.5);
                    let fuzz = rng.gen_range(0.0..0.5);
